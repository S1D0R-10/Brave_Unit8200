...
// sees a truncated key — the exact failure behind SignatureDoesNotMatch 403s.
func TestObjectURLEncodesSpecialKeyBytes(t *testing.T) {
	s := &S3Storage{endpoint: "https://store.example", bucket: "b", logger: log.Default()}

	url := s.objectURL("dir/Moduł#2 - Lekcja 4.pdf")
	want := "https://store.example/b/dir/Modu%C5%82%232%20-%20Lekcja%204.pdf"
	if url != want {
	...
	if got, wantPath := req.URL.EscapedPath(), "/b/dir/Modu%C5%82%232%20-%20Lekcja%204.pdf"; got != wantPath {
