...
// s3Sign adds AWS Signature V4 headers to an HTTP request.
// This is a minimal implementation supporting GET and HEAD for S3-compatible stores.
func s3Sign(req *http.Request, accessKey, secretKey, region, service string) {
	now := time.Now().UTC()
	datestamp := now.Format("20060102")
	amzdate := now.Format("20060102T150405Z")

	req.Header.Set("x-amz-date", amzdate)
	req.Header.Set("x-amz-content-sha256", s3PayloadHash(req))
...
// s3CanonicalHeaders builds the canonical headers string and signed headers list.
...
// s3PayloadHash returns the SHA-256 hex hash of the request body.
// For GET/HEAD requests (no body), returns the hash of empty string.
func s3PayloadHash(req *http.Request) string {
